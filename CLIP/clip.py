...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None):
    ...
    if not jit:
        ...
        if str(device) == "cpu":
            model.float()
        return model, _transform(model.visual.input_resolution), _transform2(model.visual.input_resolution), \
               _transform3(model.visual.input_resolution)
